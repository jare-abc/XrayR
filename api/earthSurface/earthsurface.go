...
func (c *APIClient) GetNodeInfo() (nodeInfo *api.NodeInfo, err error) {
	...
	path := "/api/systemnode/nodesetting"
...
func (c *APIClient) GetUserList() (UserList *[]api.UserInfo, err error) {
	var users []*user
	path := "/api/systemnode/usenodeuser"
...
func (c *APIClient) ReportUserTraffic(userTraffic *[]api.UserTraffic) error {
	path := "/api/systemnode/usedtraffic"
...
func (c *APIClient) ReportNodeStatus(nodeStatus *api.NodeStatus) (err error) {
	path := "/api/systemnode/checkonline"

	//path := fmt.Sprintf("/api/systemnode/checkonline/%d/info", c.NodeID)
...
func (c *APIClient) ReportNodeOnlineUsers(onlineUserList *[]api.OnlineUser) error {
	...
	path := "/api/systemnode/nodeonline"
...
func (c *APIClient) parseV2rayNodeResponse(s *serverConfig) (*api.NodeInfo, error) {
	...
	if s.ResultData.NodeSetting.Dest != "" {
		dest = s.ResultData.NodeSetting.Dest
	} else {
		dest = s.ResultData.NodeSetting.Sni
	}
	if s.ResultData.NodeSetting.xVer != 0 {
		xVer = s.ResultData.NodeSetting.xVer
	} else {
		xVer = 0
	}

	realityConfig := api.REALITYConfig{
		Dest:             dest + ":" + strconv.Itoa(s.ResultData.ServerPort),
		ProxyProtocolVer: xVer,
		ServerNames:      []string{s.ResultData.NodeSetting.Sni},
		PrivateKey:       s.ResultData.NodeSetting.PrivateKey,
		ShortIds:         []string{s.ResultData.NodeSetting.ShortId},
	}
	...
	switch s.ResultData.NodeSetting.Tls {
	case 0:
		enableTLS = false
		enableREALITY = false
	case 2:
	...
	return &api.NodeInfo{
		...
		VlessFlow:         s.ResultData.NodeSetting.VlessFlow,
		ServiceName:       s.ResultData.NodeSetting.ServiceName,
		Header:            header,
